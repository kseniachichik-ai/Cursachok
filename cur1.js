...
let expName = 'cur1';  // from the Builder filename that created this script
let expInfo = {
    'participant': '',
    'session': '001',
};
...
psychoJS.start({
  ...
  resources: [
    // resources:
    {'name': 'testikkk.xlsx', 'path': 'testikkk.xlsx'},
...
async function experimentInit() {
  ...
  moon_testt = new visual.ImageStim({
    ...
    image : '/Users/ksenia/Downloads/cursach_pie/g_moon.png', mask : undefined,
    anchor : 'center',
    ori : 0.0, 
    pos : [0, 0], 
    draggable: false,
  ...
  rocket_2 = new visual.ImageStim({
    ...
    image : '/Users/ksenia/Downloads/cursach_pie/rockett.png', mask : undefined,
    anchor : 'center',
    ori : 0.0, 
    pos : [0, 0], 
    draggable: false,
...
function Loop_ALoopBegin(Loop_ALoopScheduler, snapshot) {
  return async function() {
    ...
    Loop_A = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1 if expInfo['condition'] == 'A' else 0, method: TrialHandler.Method.RANDOM,
...
function Loop_BLoopBegin(Loop_BLoopScheduler, snapshot) {
  return async function() {
    ...
    Loop_B = new TrialHandler({
      psychoJS: psychoJS,
      nReps: 1 if expInfo['condition'] == 'B' else 0, method: TrialHandler.Method.RANDOM,
      extraInfo: expInfo, originPath: undefined,
      trialList: undefined,
...
function instmoonRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instmoonComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function moon_testRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of moon_testComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function moon_testRoutineEachFrame() {
  return async function () {
    ...
    keys = keyboard_space_2.getKeys(["space"], {"waitRelease": false});
    if (((keys.length > 0) && (! movementStarted))) {
        ...
        if ((((keys.length > 0) && movementStarted) && (! movementStopped))) {
            ...
            rocketCenterY = rocket_2.pos[1];
            moonCenterY = moon_testt.pos[1];
            rocketTop = (rocketCenterY + rocketHalf);
            rocketBottom = (rocketCenterY - rocketHalf);
            moonTop = (moonCenterY + moonHalf);
            moonBottom = (moonCenterY - moonHalf);
            if (((rocketTop <= moonTop) && (rocketBottom >= moonBottom))) {
                feedbackMessage = "\u041f\u043e\u043b\u0443\u0447\u0438\u043b\u043e\u0441\u044c!";
                hit = 1;
                moon_testt.color = "green";
            } else {
                if ((rocketCenterY > moonCenterY)) {
                    feedbackMessage = "\u041f\u0435\u0440\u0435\u043b\u0435\u0442";
                    hit = 0;
                    moon_testt.color = "red";
                } else {
            ...
            errorSeconds = (flightDuration - test);
...
function moon_testRoutineEnd(snapshot) {
  return async function () {
    ...
    psychoJS.experiment.addData("test_time_moon", test);
...
function in_2RoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of in_2Components)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function moonconnRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of moonconnComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function moonconnRoutineEachFrame() {
  return async function () {
    ...
    keys = keyboard_space.getKeys(["space"], {"waitRelease": false});
    if (((keys.length > 0) && (! movementStarted))) {
        movementStarted = true;
        rocketMoving = true;
        rocket.opacity = 0;
        ...
        if ((((keys.length > 0) && movementStarted) && (! movementStopped))) {
            ...
            rocket.opacity = 1;
            flightDuration = moveClock.getTime();
            rocketCenterY = rocket.pos[1];
            moonCenterY = moon.pos[1];
            rocketTop = (rocketCenterY + rocketHalf);
            rocketBottom = (rocketCenterY - rocketHalf);
            moonTop = (moonCenterY + moonHalf);
            moonBottom = (moonCenterY - moonHalf);
...
function instr_flightRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of instr_flightComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function example_testRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of example_testComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function JUSTFLIGHT_testRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of JUSTFLIGHT_testComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function JUSTFLIGHT_testRoutineEachFrame() {
  return async function () {
    ...
    keys = spaceKey_2.getKeys(["space"], {"waitRelease": false});
    if ((keys.length > 0)) {
        pressCount += 1;
        ...
        if ((pressCount === 1)) {
            ...
            if ((pressCount === 2)) {
                ...
                rockett_2.opacity = 1;
                deviation = (measuredTime - test);
                if ((measuredTime > (test + 0.0333))) {
...
function feedbackkRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of feedbackkComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function JUSTFLIGHTRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of JUSTFLIGHTComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function JUSTFLIGHTRoutineEachFrame() {
  return async function () {
    ...
    keys = spaceKey.getKeys(["space"], {"waitRelease": false});
    if ((keys.length > 0)) {
        pressCount += 1;
        spaceKey.clearEvents();
        if ((pressCount === 1)) {
